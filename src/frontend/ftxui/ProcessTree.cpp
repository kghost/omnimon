...
ProcessTree::ProcessTree(OmniMonInterface& interface)
    : _Interface(interface), _CursorRow(_Rows.end()), _Columns(CreateDefaultColumns()),
...
void ProcessTree::MoveCursorAndDraw(ssize_t offset) {
  ...
  assert(_CursorRow->Process.has_value());
...
void ProcessTree::UpdateData(backend::process::Process& selectedProcess,
  ...
  std::map<backend::process::PidType, std::list<Row>::iterator> existingProcesses;
  for (auto it = oldRows.begin(); it != oldRows.end(); ++it) {
    if (it->Process.has_value()) {
      existingProcesses.emplace(it->Process.value().get().GetPid(), it);
    }
  }
  ...
  for (auto& process : ps | std::views::transform([](auto& ref) -> backend::process::Process& { return ref.get(); })) {
    ...
    if (it != existingProcesses.end()) {
      ...
      _Rows.emplace_back(*this, process);
      for (auto& column : _Columns) {
        column->RegisterRow(_Rows.back());
      }
      _Interface.Debug("Showing new Process: [" + std::to_string(process.GetPid()) + "] " + process.GetComm(),
                       DebugWindow::DebugLevel::Info);
    }
