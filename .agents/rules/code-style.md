...
description: "C++ coding style guidelines for the OmniMon project"
---

# OmniMon C++ Code Style Guidelines

All C++ source (`.cpp`) and header (`.hpp`) files in the OmniMon repository must adhere strictly to these guidelines to ensure consistency, readability, and modern C++ practices.
...
- **Member Variables (Private or Protected):** `_PascalCase` (Preceded by a single underscore and starting with an uppercase letter)
  - *Example:* `OmniMonInterface& _Interface;`, `std::list<Row> _Rows;`
